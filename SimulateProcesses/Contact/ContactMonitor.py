...
class MonitorLocalApproximation():
	def __init__(self, initial_measure, p, q):
	...
	def run(self,steps):
		for s in range(steps):
			(diff,measure) = Local.simulate_local_approximation(1,self.current_measure, self.p, self.q)
...
class MonitorKApproximation(MonitorLocalApproximation):
	def __init__(self,initial_measure,k,p,q):
		super(MonitorKApproximation,self).__init__(initial_measure,p,q)
		self.k = k

	def run(self,steps):
		for s in range(steps):
			(diff,measure) = Local.simulate_k_approximation(1,self.current_measure, self.k, self.p, self.q)
			self.consec_diff.append(diff)
			self.measure_history.append(measure)
			self.current_measure = measure