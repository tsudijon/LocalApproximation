...
def simulate_local_approximation(steps, measure, p):
	'''
	...
	for i in range(steps):
		new_measure = apply_nonlinear_markov_operator(True, dimension,current_measure, side_length,history_length, p)
...
def simulate_k_approximation(steps, measure, k, p):
	'''
	...
	for i in range(steps):
		new_measure = apply_nonlinear_markov(measure_diff, dimension, current_measure, side_length, history_length, p)
...
def apply_nonlinear_markov(measure_diff,dimension, current_measure, side_length, history_length, p):
	'''
	...
	cond_exp = compute_conditional_expectation(side_length, current_measure, p)
	...
	for j in prange(len(states)):
		...
		for i in range(dimension):
			...
			if i == 0:
				...
				functional = stochastic_frequency_probability_of_one(current_sign, p,
								np.sum( (2*np.array(tuple(state)[1:])/side_length).astype(int) ), len(state)-1 )
...
@njit(cache=True)
def compute_conditional_expectation(side_length, current_measure, p):
	'''
	...
	for i in range(side_length):
		for j in range(side_length):
			...
			weighted_values = [prob*stochastic_frequency_probability_of_one(current_sign, p, 
							np.sum((2*np.array(idx[1:])/side_length).astype(np.int32)), len(idx) - 1) for idx, prob in \
...
@njit(cache=True)
def stochastic_frequency_probability_of_one(current_sign, p, sum_nbrs, length):
	q = (1-p)/2
	
	if current_sign == 0:
		prob = q + p*(length - sum_nbrs)/length
		return prob
	else:
		prob = q + p*sum_nbrs/length
		return 1 - prob
