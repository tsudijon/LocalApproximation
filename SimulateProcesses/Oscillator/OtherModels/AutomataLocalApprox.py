	...
	for j in prange(len(states)):
		...
		for i in range(dimension):
			...
			if i == 0:
				...
				functional = probability_of_one(current_sign, p,
...
def compute_conditional_expectation(side_length, current_measure, p):
	...
	for i in range(side_length):
		for j in range(side_length):
			...
			weighted_values = [prob*probability_of_one(current_sign, p, 
...
@njit(cache=True)
def probability_of_one(current_sign, p, sum_neighbors,len_neighbors):
	'''
	compute probability of transitioning to the one state
	'''

	if current_sign == 1:
		probability = (len_neighbors - sum_neighbors)*p/len_neighbors
	else:
		probability = 1 - sum_neighbors*p/len_neighbors
