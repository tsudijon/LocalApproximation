...
sys.path.append("../SimulateProcesses/Voter")
import Create_Graphs as graphs 
import Dyadic_measures as measures
import Voter_Model as Voter 
...
def run_k_approximation(depth,kappa, T, k, bias, load_data = False):
	"""
	...
	f = "VoterModelResults/kapprox/kapprox_kappa{}_T{}_k{}_bias{}.npy".format(kappa,T,k,bias)
	...
	process = monitor.MonitorKApproximation(initial,k)
...
def run_local_approximation(depth,kappa, T, bias, load_data = False):
	"""
	...
	f = "VoterModelResults/localapprox/localapprox_kappa{}_T{}_bias{}.npy".format(kappa,T,bias)
	...
	process = monitor.MonitorLocalApproximation(initial)
...
def run_dynamics(depth, kappa, T, bias, bd_condition, load_data = False, use_bc = False):
	"""
	...
	f = "VoterModelResults/dynamics/dynamics_kappa{}_depth{}_T{}_bias{}.npy".format(kappa,depth,T,bias)

	jointf = "VoterModelResults/dynamics/joint_dynamics_kappa{}_depth{}_T{}_bias{}.npy".format(kappa,depth,T,bias)
	...
	if load_data == False:
		...
		probs = Misc.convert_to_zero_one(Voter.simulate_voter_model_dynamics(graph,T,bd_condition,bias, use_bc)[0])
		...
		for i in range(N-1):
			dynamics = Misc.convert_to_zero_one(Voter.simulate_voter_model_dynamics(graph,T,bd_condition,bias, use_bc)[0]) 
			for j in range(len(probs)):
				probs[j] = Misc.append_dicts(probs[j],dynamics[j]) 							
		...
		np.save(f,prob_means)
		np.save(jointf,joint_means)

		np.save(f + "_std",prob_std)
		np.save(jointf + "_std", joint_std)

	else:
		prob_means = np.load(f)
		joint_means = np.load(jointf)

		
		prob_std = np.load(f + "_std")
		joint_std = np.load(jointf + "_std")
...
def run_mean_field_simulations(kappa,p_0,T,p,q, load_data = False):
	...
	f = "VoterModelResults/meanfield/meanfield_kappa{}_p0{}_T{}.npy".format(kappa,p_0,p,q,T)
	if load_data == False:
		probs = Voter.simulate_mean_field_dynamics(kappa,p_0,T,p,q)
		np.save(f,probs)
	else:
		probs = np.load(f)
...
def plot_vs_kappa(df,error_df, kappa_params, depth_params, k_params, title, scale_y = False):
...
def plot_joint_vs_kappa(df,error_df, kappa_params, depth_params, k_params, title, state, scale_y = False):
...
def plot_prob_vs_time(df,error_df, kappa, depth, title, scale_y = False):
	idx = pd.IndexSlice
	plt.figure(figsize = (5,5))
	ax = plt.gca()
...
def plot_joint_state_prob_vs_time(df, error_df, kappa, depth, state, title, scale_y = False):
...
if __name__== "__main__":
	...
	for kappa, depth in itr.product(kappa_params,depth_params):
		...
		(root, root_std, joint, joint_std, root_occ, root_occ_std, joint_occ,
		 joint_occ_std, root_history,root_std_history, joint_history, joint_std_history) = \
			run_dynamics(depth,kappa,T,bias,bd_condition,load_data = False, use_bc = False)
	...
	for kappa in kappa_params:
		...
		(root, joint, root_occ, joint_occ, root_history, joint_history) = \
			run_mean_field_simulations(kappa,p_0,T, load_data = False)
	...
	for k,kappa in itr.product(k_params,kappa_params):
		...
		(root, joint, root_occ, joint_occ, root_history, joint_history) = \
			run_k_approximation(1, kappa, T, k, bias, load_data = False) 
	...
	# save the dataframes
	def get_df_string(s):
		f = "VoterModelResults/dataframes/{}_T{}_maxkappa{}_maxdepth{}_k{}_bias{}".format(s,T,
	...
	plot_joint_vs_kappa(joint_prob_df, joint_prob_error_df, kappa_params, 
		depth_params[1:], k_params,"Probability of State {}".format(state), state, False)

	# joint occupation measure, for a given state
	plot_joint_vs_kappa(joint_occ_df, joint_occ_error_df, kappa_params, 
		depth_params[1:], k_params, "Expected Time at State {}".format(state), state, False)
	...
	plot_prob_vs_time(single_node_history_df,single_node_error_history_df, kappa,depth,"Voter Model", False)

	plot_joint_state_prob_vs_time(joint_history_df,joint_error_history_df, kappa,depth, state, "Voter Model", False)
