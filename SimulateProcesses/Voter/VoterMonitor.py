import JittedVoterLocalApprox as Local
...
class MonitorLocalApproximation():
	...
	def run(self,steps):
		for s in range(steps):
			(diff,measure) = Local.simulate_local_approximation(1,self.current_measure)
...
class MonitorKApproximation(MonitorLocalApproximation):
	...
	def run(self,steps):
		for s in range(steps):
			(diff,measure) = Local.simulate_k_approximation(1,self.current_measure, self.k)
			self.consec_diff.append(diff)
			self.measure_history.append(measure)
			self.current_measure = measure