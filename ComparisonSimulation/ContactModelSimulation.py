...
def run_dynamics(depth, kappa, T, p, q, bias, bd_condition, load_data = False, use_bc = False):
	...
	if load_data == False:
		...
		#graph = graphs.complete_graph(kappa + 1)
		#graph = nx.cycle_graph(depth)

		# get probabilities by averaging over histories
		probs = Contact.simulate_Contact_Process_dynamics(graph,T,p,q,bd_condition,bias, use_bc)[0]
		...
		for i in range(N-1):
			...
			dynamics = Contact.simulate_Contact_Process_dynamics(graph,T,p,q,bd_condition,bias, use_bc)[0]
...
def plot_vs_kappa(df,error_df, kappa_params, depth_params, k_params, title, filename, scale_y = False, show_plot = False, yaxis = 'Probability'):
...
def plot_joint_vs_kappa(df,error_df, kappa_params, depth_params, k_params, title, state, filename, scale_y = False, show_plot = False, yaxis = 'Probability'):
...
if __name__== "__main__":
	...
	plot_vs_kappa(single_node_prob_df, single_node_prob_error_df, kappa_params, depth_params,
		k_params, "Root Node Probability of 1", file, False, False)

	# occupation measure
	file = "{}_{}_q{}_p{}_bias{}.jpg".format(filename,"root_occ", q, p, bias)
	plot_vs_kappa(single_node_occ_df, single_node_occ_error_df, kappa_params, depth_params,
	...
	for kappa in kappa_params:
		for depth in depth_params:
			...
			plot_prob_vs_time(single_node_history_df,single_node_error_history_df, kappa,depth,k_params, "Contact Process", file, False, False)
	...
	for state in [(0,0),(0,1),(1,0),(1,1)]:
		...
		for kappa in kappa_params:
			for depth in depth_params:
				...
				plot_joint_state_prob_vs_time(joint_history_df,joint_error_history_df, kappa, depth, k_params, state, "Contact Process", file, False, False)
