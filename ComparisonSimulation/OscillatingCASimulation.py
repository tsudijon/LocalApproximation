#######################################################
# This simulation analyzes performance of the Oscillating CA model
...
import OscillatingCA as oscillator
import OscillatingCAMonitor as monitor
import Misc
...
def run_dynamics(depth, kappa, T, p, bias, load_data = False):
	...
	if load_data == False:
		...
		probs = oscillator.simulate_stochastic_frequency_dynamics(graph, T, p, bias)
		...
		for i in range(N-1):
			print(i)
			dynamics = oscillator.simulate_stochastic_frequency_dynamics(graph, T, p, bias)
...
def run_mean_field_simulations(kappa,p_0,T,p, load_data = False):
	...
	if load_data == False:
		probs = oscillator.simulate_mean_field_dynamics(T, kappa, p, p_0)
		np.save(f,probs)
	else:
		probs = np.load(f)
	print(probs)
...
def plot_joint_state_prob_vs_time(df, error_df, kappa, depth, k_params, state, title, filename, scale_y = False, show_plot = False):
	"""
	Plots dataframe of joint probabilities against time for fixed values of kappa and depth, with error bars
