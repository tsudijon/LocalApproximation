...
def run_k_approximation(depth,kappa, T,p ,beta, k, bias, load_data = False):
	"""
	...
	f = "IsingModelResults/kapprox/kapprox_kappa{}_p{}_beta{}_T{}_k{}_bias{}.npy".format(kappa,p,beta,T,k,bias)
	...
	process = monitor.MonitorKApproximation(initial,k,p,beta)
...
def run_local_approximation(depth,kappa, T,p ,beta, bias, load_data = False):
	"""
	...
	f = "IsingModelResults/localapprox/localapprox_kappa{}_p{}_beta{}_T{}_bias{}.npy".format(kappa,p,beta,T,bias)
	...
	process = monitor.MonitorLocalApproximation(initial,p,beta)
...
def run_dynamics(depth, kappa, T, p, beta, bias, bd_condition,  load_data = False, use_bc = False):
	"""
	...
	f = "IsingModelResults/dynamics/dynamics_kappa{}_depth{}_p{}_beta{}_T{}_bias{}.npy".format(kappa,depth,p,beta,T,bias)

	jointf = "IsingModelResults/dynamics/joint_dynamics_kappa{}_depth{}_p{}_beta{}_T{}_bias{}.npy".format(kappa,depth,p,beta,T,bias)
	...
	if load_data == False:
		...
		probs = Misc.convert_to_zero_one(Ising.simulate_parallel_glauber_dynamics(graph,T,p,beta,bd_condition,bias, use_bc)[0])
		...
		for i in range(N-1):
			print(i)
			dynamics = Misc.convert_to_zero_one(Ising.simulate_parallel_glauber_dynamics(graph,T,p,beta,bd_condition,bias, use_bc)[0]) 
...
def run_mean_field_simulations(kappa,p_0,T,p,beta, load_data = False):

	f = "IsingModelResults/meanfield/meanfield_kappa{}_p0{}_p{}_beta{}_T{}.npy".format(kappa,p_0,p,beta,T)
	if load_data == False:
		probs = Ising.simulate_mean_field_dynamics(kappa,p_0,T,p,beta)
		np.save(f,probs)
	else:
		probs = np.load(f)
...
def plot_prob_vs_time(df,error_df, kappa, depth, title, filename, scale_y = False, show_plot = False):
	idx = pd.IndexSlice
	plt.figure(figsize = (5,5))
	ax = plt.gca()
...
def plot_joint_state_prob_vs_time(df, error_df, kappa, depth, state, title, filename, scale_y = False, show_plot = False):
...
if __name__== "__main__":
	...
	for kappa, depth in itr.product(kappa_params,depth_params):
		...
		if kappa == 2:
			bd_condition = np.random.choice([-1,1],2)
		else:
			bd_condition = np.random.choice([-1,1],(kappa**depth))
		...
		(root, root_std, joint, joint_std, root_occ, root_occ_std, joint_occ,
		 joint_occ_std, root_history,root_std_history, joint_history, joint_std_history) = \
			run_dynamics(depth,kappa,T,p,beta,bias,bd_condition,load_data = False, use_bc = False)
	...
	file = "{}_{}_beta{}_p{}_bias{}.jpg".format(filename,"root_prob", beta, p, bias)
	...
	# occupation measure
	file = "{}_{}_beta{}_p{}_bias{}.jpg".format(filename,"root_occ", beta, p, bias)
	...
	for kappa in kappa_params:
		for depth in depth_params:
			file = "{}_{}_beta{}_p{}_bias{}_kappa{}_depth{}.jpg".format(filename,"root_prob_vs_time", beta, p, bias, kappa, depth)
			plot_prob_vs_time(single_node_history_df,single_node_error_history_df, kappa,depth,"Ising Model", file, False, False)

	# joint probability, for a given state
	for state in [(0,0),(0,1),(1,0),(1,1)]:
		file = "{}_{}_beta{}_p{}_bias{}_state{}.jpg".format(filename,"joint_prob",
										 beta, p, bias,state)
		...
		file = "{}_{}_beta{}_p{}_bias{}_state{}.jpg".format(filename,"joint_occ",
										 beta, p, bias, state)
		...
		for kappa in kappa_params:
			for depth in depth_params:
				file = "{}_{}_beta{}_p{}_bias{}_kappa{}_depth{}_state{}.jpg".format(filename,"joint_occ",
										 beta, p, bias, kappa, depth, state)
				plot_joint_state_prob_vs_time(joint_history_df,joint_error_history_df, kappa,depth, state, "Ising Model", file, False, False)
